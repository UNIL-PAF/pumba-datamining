...
neg_label <- "no phospho"
pos_label <- "phospho"

# load results
res_path <- ("/Users/admin/tmp/datamining_pumba/results/ptm_1565189609.54642.RData")
load(res_path)
...
for(sample in names(results)){
  ...
  peak_dists$color[grep(pos_label, peak_dists$ptms, ignore.case=TRUE)] <- pos_label
